...
Para rodar como serviço Windows, use NSSM ou configure no Task Scheduler.
"""
...
BACKEND_URL = "https://order-print-1.preview.emergentagent.com"

# API Key do Print Agent (obtenha no Admin > Definições > Print Agent)
...
class PrintAgent:
    ...
    def process_job(self, job_data: dict) -> bool:
        ...
        if is_test:
            ...
            logger.info(f"Processando pedido #{order.get('order_number')} para {printer_name}")
            print_data = printer.format_order(order, printer_name, restaurant_name)
        else:
        ...
        if success:
            logger.info(f"Job {job_id}: Impresso com sucesso em {printer_name}")
...
╚══════════════════════════════════════════════════════════════╝
    ...
    if API_KEY == "SUA_API_KEY_AQUI":
        ...
        print("2. Vá em Definições > Print Agent")
